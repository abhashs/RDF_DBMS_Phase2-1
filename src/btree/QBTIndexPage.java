 ...
 */
public class QBTIndexPage extends QBTSortedPage{
   ...
   */
  public QBTIndexPage(PageId pageno, int keyType) 
    throws IOException, 
   ...
   *@exception ConstructPageException error when BTIndexpage constructor
   */
  public QBTIndexPage(Page page, int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super(page, keyType);
   ...
   *@exception ConstructPageException error when BTIndexpage constructor
   */
  public QBTIndexPage( int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super(keyType);
  ...
  RID deleteKey(KeyClass key) 
	...
	while (QBT.keyCompare(key, entry.key) > 0) {
	  entry = getNext(rid );
	  if (entry == null)
            break;
	}
	...
	else if (QBT.keyCompare(key, entry.key) != 0)
  ...
  PageId getPageNoByKey(KeyClass key) 
	...
	for (i=getSlotCnt()-1; i >= 0; i--) {
	  entry= QBT.getEntryFromBytes( getpage(),getSlotOffset(i), 
  ...
  public KeyDataEntry getFirst(RID rid) 
	...
	entry=QBT.getEntryFromBytes( getpage(),getSlotOffset(0), 
				    getSlotLength(0),
  ...
  int  getSibling(KeyClass key, PageId pageNo)
	...
	for (i=getSlotCnt()-1; i >= 0; i--) {
	  ...
	  if (QBT.keyCompare(key, entry.key)>=0) {
	    if (i != 0) { 
              entry=QBT.getEntryFromBytes(getpage(), getSlotOffset(i-1),
   ...
   */
  boolean redistribute(QBTIndexPage indexPage, QBTIndexPage parentIndexPage,
	...
	if (direction==-1) { // 'this' is the left sibling of indexPage
	  if (( getSlotLength(getSlotCnt()-1) + available_space()) > 
	      ( (MAX_SPACE-DPFIXED)/2) ) {
            ...
            lastEntry=QBT.getEntryFromBytes(getpage(), getSlotOffset(getSlotCnt()-1),     
	  ...
	  if ( (getSlotLength(0) + available_space()) > ((MAX_SPACE-DPFIXED)/2) ) {
            ...
            firstEntry=QBT.getEntryFromBytes( getpage(),
					     getSlotOffset(0),
					     getSlotLength(0), keyType, NodeType.INDEX);
