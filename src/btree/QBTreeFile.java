...
/** btfile.java
 * This is the main definition of class QBTreeFile, which derives from 
...
public class QBTreeFile extends QIndexFile 
  ...
  private QBTreeHeaderPage headerPage;
  private  PageId  headerPageId;
  private String  dbname;  
  
  /**
   * Access method to data member.
   * @return  Return a QBTreeHeaderPage object that is the header page
   *          of this btree file.
   */
  public QBTreeHeaderPage getHeaderPage() {
    return headerPage;
  }
   ...
   *@exception ConstructPageException   QBT page constructor failed
   */
  public QBTreeFile(String filename)
    throws GetFileEntryException,  
	   PinPageException, 
    ...
    {      
      ...
      headerPage= new  QBTreeHeaderPage( headerPageId);       
      dbname = new String(filename);
      /*
       *
       * - headerPageId is the PageId of this QBTreeFile's header page;
   ...
   */
  public QBTreeFile(String filename, int keytype,
	...
	{
	  headerPage= new  QBTreeHeaderPage(); 
   ...
   *@exception ConstructPageException error in QBT page constructor 
   *@exception PinPageException failed when pin a page
   */
  ...
  private void  _destroyFile(PageId pageno) 
	   ...
	   FreePageException
    {
      ...
      sortedPage= new QBTSortedPage( page, headerPage.get_keyType());
  ...
  private void  updateHeader(PageId newRoot)
	     ...
	     UnpinPageException
    {
      ...
      QBTreeHeaderPage header;
      PageId old_data;
      
      
      header= new QBTreeHeaderPage( pinPage(headerPageId));
  ...
  public void insert(KeyClass key, QID rid) 
	   ...
	   
    {
      ...
      if (QBT.getKeyLength(key) > headerPage.get_maxKeySize())
      ...
      if (newRootEntry != null)
	{
	  ...
	  newRootPage = new QBTIndexPage(headerPage.get_keyType());
	  newRootPageId=newRootPage.getCurPage();
  ...
  private KeyDataEntry  _insert(KeyClass key, QID rid,  
	    ...
	    
    {
      ...
      QBTSortedPage currentPage;
      Page page;
      KeyDataEntry upEntry;
      
      
      page=pinPage(currentPageId);
      currentPage=new QBTSortedPage(page, headerPage.get_keyType());      
	...
	QBTIndexPage  currentIndexPage=new QBTIndexPage(page, 
	...
	currentIndexPage= new  QBTIndexPage(pinPage(currentPageId),
					   headerPage.get_keyType() );
	...
	if (currentIndexPage.available_space() >=
	    QBT.getKeyDataLength( upEntry.key, NodeType.INDEX))
	  {
	...
	if (QBT.keyCompare( upEntry.key, tmpEntry.key) >=0 )
	  {
	  ...
	  tmpEntry =
	    QBT.getEntryFromBytes(currentIndexPage.getpage(), 
	...
	{
	  QBTLeafPage currentLeafPage = 
	    new QBTLeafPage(page, headerPage.get_keyType() );
	  ...
	  if (currentLeafPage.available_space() >=
	      QBT.getKeyDataLength(key, NodeType.LEAF) )
	    {
	      // no split has occurred
	  ...
	  if (rightPageId.pid != INVALID_PAGE)
	    {
	      QBTLeafPage rightPage;
	      rightPage=new QBTLeafPage(rightPageId, headerPage.get_keyType());
	  ...
	  if (QBT.keyCompare(key, undoEntry.key ) <  0) {
	    //undo the final record
	    if ( currentLeafPage.available_space() < 
		 newLeafPage.available_space()) {
   ...
   *@exception FreePageException error in QBT page constructor
   *@exception RecordNotFoundException error delete a record in a QBT page
   *@exception PinPageException error when pin a page
   ...
   *@return return a QBTLeafPage instance which is pinned. 
  ...
  QBTLeafPage findRunStart (KeyClass lo_key, 
    ...
    throws IOException, 
	   ...
	   UnpinPageException
    {
      QBTLeafPage  pageLeaf;
      QBTIndexPage pageIndex;
      Page page;
      QBTSortedPage  sortPage;
      PageId pageno;
      ...
      if (pageno.pid == INVALID_PAGE){        // no pages in the QBTREE
      ...
      page= pinPage(pageno);
      sortPage=new QBTSortedPage(page, headerPage.get_keyType());
	...
	if ( trace!=null )
      ...
      pageLeaf = new QBTLeafPage(page, headerPage.get_keyType() );
	...
	pageno = nextpageno; 
	pageLeaf=  new QBTLeafPage( pinPage(pageno), headerPage.get_keyType());    
	curEntry=pageLeaf.getFirst(startrid);
      }
	...
	// scan will unpin it when done
      ...
      while (QBT.keyCompare(curEntry.key, lo_key) < 0) {
  ...
  private boolean NaiveDelete ( KeyClass key, QID rid)
	...
	leafPage=new QBTLeafPage(pinPage(nextpage), headerPage.get_keyType());
  ...
  private KeyClass _Delete ( KeyClass key,
	...
	while((tmpEntry!=null) && (QBT.keyCompare(key,tmpEntry.key)>=0)) { 
	  ...
	  if ( leafPage.delEntry(new KeyDataEntry(key, rid)) ) {
		...
		return null;
	      ...
	      // get a sibling
	      QBTIndexPage  parentPage;
	      parentPage =
		new QBTIndexPage(pinPage(parentPageId), 
				headerPage.get_keyType());
	      
	      PageId siblingPageId=new PageId();
	      QBTLeafPage siblingPage;
	      int direction;
		...
		return null;
              ...
              siblingPage=new QBTLeafPage(pinPage(siblingPageId), 
					 headerPage.get_keyType());
		...
		QBTLeafPage leftChild, rightChild;
		if (direction==-1) {
		...
		if ( rightChild.getNextPage().pid != INVALID_PAGE) {
		  QBTLeafPage nextLeafPage=new QBTLeafPage(
	...
	indexPage=new QBTIndexPage(pinPage(currentPageId), headerPage.get_keyType());
	...
	if (indexPage.getCurPage().pid == headerPage.get_rootId().pid) {
	  ...
	  if (indexPage.numberOfRecords() == 0) {
	    QBTSortedPage childPage;
	    childPage=new QBTSortedPage(indexPage.getPrevPage(),
				       headerPage.get_keyType()); 
	...
	if ((4 /*sizeof slot*/ +indexPage.available_space()) <= 
	  ...
	  // get a sibling
	  QBTIndexPage  parentPage;
	  parentPage=new QBTIndexPage(pinPage(parentPageId), 
				     headerPage.get_keyType()); 
	  
	  PageId siblingPageId=new PageId();
	  QBTIndexPage siblingPage;
	  int direction;
	  ...
	  siblingPage=new QBTIndexPage( pinPage(siblingPageId), 
				       headerPage.get_keyType());
	  ...
	  if (siblingPage.redistribute(indexPage,parentPage, 
	    ...
	    // merge the two children
	    QBTIndexPage leftChild, rightChild;
	    if (direction==-1) {
	      leftChild = siblingPage;
   ...
   *@exception UnpinPageException error when unpin a page
   */
  public QBTFileScan new_scan(KeyClass lo_key, KeyClass hi_key)
    throws IOException,  
	   KeyNotMatchException, 
	   IteratorException, 
	   ConstructPageException, 
    ...
    {
      QBTFileScan scan = new QBTFileScan();
      if ( headerPage.get_rootId().pid==INVALID_PAGE) {
	scan.leafPage=null;
	return scan;
      }
  ...
  void trace_children(PageId id)
	...
	KeyClass key;
	KeyDataEntry entry;
	sortedPage=new QBTSortedPage( pinPage( id), headerPage.get_keyType());
	
	
	// Now print all the child nodes of the page.  
	if( sortedPage.getType()==NodeType.INDEX) {
	  QBTIndexPage indexPage=new QBTIndexPage(sortedPage,headerPage.get_keyType()); 
	  ...
	  QBTLeafPage leafPage=new QBTLeafPage(sortedPage,headerPage.get_keyType()); 
