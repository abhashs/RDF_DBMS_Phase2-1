 ...
 */
public class LBTSortedPage  extends LHFPage{
   ...
   */
  public LBTSortedPage(PageId pageno, int keyType) 
   ...
   *               AttrType.attrString or AttrType.attrInteger. 
   */
  public LBTSortedPage(Page page, int keyType) {
   ...
   *@exception  ConstructPageException error for BTSortedPage constructor
   */ 
  public LBTSortedPage(int keyType) 
    throws ConstructPageException
    {
      super();
      try{
   ...
   */
          ...
          throws InsertRecException 
   {
     ...
     try {
       ...
       record=LBT.getBytesFromEntry(entry);  
       rid=super.insertRecord(record);
         if (rid==null) return null;
	 
         if ( entry.data instanceof LLeafData )
	   nType= NodeType.LEAF;
	 ...
	 for (i=getSlotCnt()-1; i > 0; i--) 
	   {
	     ...
	     key_i=LBT.getEntryFromBytes(getpage(), getSlotOffset(i), 
					getSlotLength(i), keyType, nType).key;
	     
	     key_iplus1=LBT.getEntryFromBytes(getpage(), getSlotOffset(i-1), 
					     getSlotLength(i-1), keyType, nType).key;
	     
	     if (LBT.keyCompare(key_i, key_iplus1) < 0)
	       {
	       // switch slots:
		 int ln, off;
		 ln= getSlotLength(i);
   ...
   */
  public  boolean deleteSortedRecord(LID rid)
