 ...
 */
public class QBTLeafPage extends QBTSortedPage {
   ...
   */
  public QBTLeafPage(PageId pageno, int keyType) 
    throws IOException, 
   ...
   *@exception ConstructPageException BTLeafPage constructor error
   */
  public QBTLeafPage(Page page, int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super(page, keyType);
   ...
   *@exception ConstructPageException BTLeafPage constructor error
   */
  public QBTLeafPage( int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super(keyType);
  ...
  public KeyDataEntry getFirst(QID rid) 
    throws  IteratorException
    {
      ...
      try {
        ...
        entry=QBT.getEntryFromBytes(getpage(), getSlotOffset(0), getSlotLength(0),
     ...
     throws  IteratorException
   {
	 ...
	 return null;
       ...
       entry=QBT.getEntryFromBytes(getpage(),getSlotOffset(i), getSlotLength(i),
                  keyType, NodeType.LEAF);
   ...
   */
  boolean redistribute(QBTLeafPage leafPage, QBTIndexPage parentIndexPage, 
	...
	if (direction ==-1) { // 'this' is the left sibling of leafPage
	  ...
	  if ( (getSlotLength(0) + available_space()+ 8) > ((MAX_SPACE-DPFIXED)/2)) {
            ...
            KeyDataEntry firstEntry;
            firstEntry=QBT.getEntryFromBytes(getpage(), getSlotOffset(0),
