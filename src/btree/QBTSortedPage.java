...
public class QBTSortedPage  extends THFPage{
   ...
   */
  public QBTSortedPage(PageId pageno, int keyType) 
   ...
   *               AttrType.attrString or AttrType.attrInteger. 
   */
  public QBTSortedPage(Page page, int keyType) {
   ...
   *@exception  ConstructPageException error for BTSortedPage constructor
   */ 
  public QBTSortedPage(int keyType) 
    throws ConstructPageException
    {
      super();
      try{
   ...
   */
          ...
          throws InsertRecException 
   {
     ...
     try {
       ...
       record=QBT.getBytesFromEntry(entry);  
	 ...
	 for (i=getSlotCnt()-1; i > 0; i--) 
	   {
	     ...
	     key_i=QBT.getEntryFromBytes(getpage(), getSlotOffset(i), 
					getSlotLength(i), keyType, nType).key;
	     
	     key_iplus1=QBT.getEntryFromBytes(getpage(), getSlotOffset(i-1), 
					     getSlotLength(i-1), keyType, nType).key;
	     
	     if (QBT.keyCompare(key_i, key_iplus1) < 0)
	       {
	       // switch slots:
		 int ln, off;
		 ln= getSlotLength(i);
