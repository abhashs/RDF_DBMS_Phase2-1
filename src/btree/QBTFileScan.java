 ...
 * class IndexFileScan.  
 */
public class QBTFileScan  extends IndexFileScan
             implements  GlobalConst
{
    ...
    try {
	 ...
	 if (nextpage.pid == INVALID_PAGE) {
	    ...
	    return null;
         ...
         leafPage=new QBTLeafPage(nextpage, keyType);
	 	
	 entry=leafPage.getFirst(curRid);
      }

      if (endkey != null)  
        if ( QBT.keyCompare(entry.key, endkey)  > 0) {
  ...
  /** max size of the key
   *@return the maxumum size of the key in QBTFile
   */
  public int keysize() {
    return maxKeysize;
  }  
