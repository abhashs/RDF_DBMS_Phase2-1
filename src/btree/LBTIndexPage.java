 ...
 */
public class LBTIndexPage extends LBTSortedPage{
   ...
   */
  public LBTIndexPage(PageId pageno, int keyType) 
    throws IOException, 
   ...
   *@exception ConstructPageException error when BTIndexpage constructor
   */
  public LBTIndexPage(Page page, int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super(page, keyType);
   ...
   *@exception ConstructPageException error when BTIndexpage constructor
   */
  public LBTIndexPage( int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super(keyType);
  ...
  LID deleteKey(KeyClass key) 
	...
	while (LBT.keyCompare(key, entry.key) > 0) {
	  entry = getNext(rid );
	  if (entry == null)
            break;
	}
	...
	else if (LBT.keyCompare(key, entry.key) != 0)
  ...
  PageId getPageNoByKey(KeyClass key) 
	...
	for (i=getSlotCnt()-1; i >= 0; i--) {
	  entry= LBT.getEntryFromBytes( getpage(),getSlotOffset(i), 
  ...
  public KeyDataEntry getFirst(LID rid) 
	...
	entry=LBT.getEntryFromBytes( getpage(),getSlotOffset(0), 
				    getSlotLength(0),
  ...
  int  getSibling(KeyClass key, PageId pageNo)
	...
	for (i=getSlotCnt()-1; i >= 0; i--) {
	  ...
	  if (LBT.keyCompare(key, entry.key)>=0) {
	    if (i != 0) { 
              entry=LBT.getEntryFromBytes(getpage(), getSlotOffset(i-1),
   ...
   */
  boolean redistribute(LBTIndexPage indexPage, LBTIndexPage parentIndexPage,
	...
	if (direction==-1) { // 'this' is the left sibling of indexPage
	  if (( getSlotLength(getSlotCnt()-1) + available_space()) > 
	      ( (MAX_SPACE-DPFIXED)/2) ) {
            ...
            lastEntry=LBT.getEntryFromBytes(getpage(), getSlotOffset(getSlotCnt()-1),     
	  ...
	  if ( (getSlotLength(0) + available_space()) > ((MAX_SPACE-DPFIXED)/2) ) {
            ...
            firstEntry=LBT.getEntryFromBytes( getpage(),
					     getSlotOffset(0),
					     getSlotLength(0), keyType, NodeType.INDEX);
