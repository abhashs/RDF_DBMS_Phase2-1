 ...
 * class IndexFileScan.  
 */
public class LBTFileScan  extends IndexFileScan
             implements  GlobalConst
{

  LBTreeFile bfile; 
  String treeFilename;     // B+ tree we're scanning 
  LBTLeafPage leafPage;   // leaf page containing current record
  LID curRid;       // position in current leaf; note: this is 
    ...
    try {
	 ...
	 if (nextpage.pid == INVALID_PAGE) {
	    ...
	    return null;
         ...
         leafPage=new LBTLeafPage(nextpage, keyType);
	 	
	 entry=leafPage.getFirst(curRid);
      }

      if (endkey != null)  
        if ( LBT.keyCompare(entry.key, endkey)  > 0) {
  ...
  /** max size of the key
   *@return the maxumum size of the key in LBTFile
   */
  public int keysize() {
    return maxKeysize;
  }  
