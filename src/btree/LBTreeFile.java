...
/** btfile.java
 * This is the main definition of class LBTreeFile, which derives from 
...
public class LBTreeFile extends LIndexFile 
  ...
  private LBTreeHeaderPage headerPage;
  private  PageId  headerPageId;
  private String  dbname;  
  
  /**
   * Access method to data member.
   * @return  Return a LBTreeHeaderPage object that is the header page
   *          of this btree file.
   */
  public LBTreeHeaderPage getHeaderPage() {
    return headerPage;
  }
   ...
   *@exception ConstructPageException   LBT page constructor failed
   */
  public LBTreeFile(String filename)
    throws GetFileEntryException,  
	   PinPageException, 
    ...
    {      
      ...
      /*
       *
       * - headerPageId is the PageId of this LBTreeFile's header page;
   ...
   */
  public LBTreeFile(String filename, int keytype,
	...
	{
	  headerPage= new  LBTreeHeaderPage(); 
   ...
   *@exception ConstructPageException error in LBT page constructor 
   *@exception PinPageException failed when pin a page
   */
  ...
  private void  _destroyFile(PageId pageno) 
	   ...
	   FreePageException
    {
      ...
      sortedPage= new LBTSortedPage( page, headerPage.get_keyType());
  ...
  private void  updateHeader(PageId newRoot)
	     ...
	     UnpinPageException
    {
      ...
      LBTreeHeaderPage header;
      PageId old_data;
      
      
      header= new LBTreeHeaderPage( pinPage(headerPageId));
  ...
  public void insert(KeyClass key, LID rid) 
	   ...
	   
    {
      ...
      if (LBT.getKeyLength(key) > headerPage.get_maxKeySize())
      ...
      if (newRootEntry != null)
	{
	  ...
	  newRootPage = new LBTIndexPage(headerPage.get_keyType());
	  newRootPageId=newRootPage.getCurPage();
  ...
  private KeyDataEntry  _insert(KeyClass key, LID rid,  
	    ...
	    
    {
      ...
      LBTSortedPage currentPage;
      Page page;
      KeyDataEntry upEntry;
      
      
      page=pinPage(currentPageId);
      currentPage=new LBTSortedPage(page, headerPage.get_keyType());      
	...
	LBTIndexPage  currentIndexPage=new LBTIndexPage(page, 
	...
	currentIndexPage= new  LBTIndexPage(pinPage(currentPageId),
					   headerPage.get_keyType() );
	...
	if (currentIndexPage.available_space() >=
	    LBT.getKeyDataLength( upEntry.key, NodeType.INDEX))
	  {
	...
	if (LBT.keyCompare( upEntry.key, tmpEntry.key) >=0 )
	  {
	  ...
	  tmpEntry =
	    LBT.getEntryFromBytes(currentIndexPage.getpage(), 
	...
	{
	  LBTLeafPage currentLeafPage = 
	    new LBTLeafPage(page, headerPage.get_keyType() );
	  ...
	  if (currentLeafPage.available_space() >=
	      LBT.getKeyDataLength(key, NodeType.LEAF) )
	    {
	      // no split has occurred
	  ...
	  if (rightPageId.pid != INVALID_PAGE)
	    {
	      LBTLeafPage rightPage;
	      rightPage=new LBTLeafPage(rightPageId, headerPage.get_keyType());
	  ...
	  if (LBT.keyCompare(key, undoEntry.key ) <  0) {
	    //undo the final record
	    if ( currentLeafPage.available_space() < 
		 newLeafPage.available_space()) {
   ...
   *@exception FreePageException error in LBT page constructor
   *@exception RecordNotFoundException error delete a record in a LBT page
   *@exception PinPageException error when pin a page
   ...
   *@return return a LBTLeafPage instance which is pinned. 
  ...
  LBTLeafPage findRunStart (KeyClass lo_key, 
    ...
    throws IOException, 
	   ...
	   UnpinPageException
    {
      LBTLeafPage  pageLeaf;
      LBTIndexPage pageIndex;
      Page page;
      LBTSortedPage  sortPage;
      PageId pageno;
      ...
      if (pageno.pid == INVALID_PAGE){        // no pages in the LBTREE
      ...
      page= pinPage(pageno);
      sortPage=new LBTSortedPage(page, headerPage.get_keyType());
	...
	if ( trace!=null )
      ...
      pageLeaf = new LBTLeafPage(page, headerPage.get_keyType() );
	...
	pageno = nextpageno; 
	pageLeaf=  new LBTLeafPage( pinPage(pageno), headerPage.get_keyType());    
	curEntry=pageLeaf.getFirst(startrid);
      }
	...
	// scan will unpin it when done
      ...
      while (LBT.keyCompare(curEntry.key, lo_key) < 0) {
  ...
  private boolean NaiveDelete ( KeyClass key, LID rid)
	...
	leafPage=new LBTLeafPage(pinPage(nextpage), headerPage.get_keyType());
  ...
  private KeyClass _Delete ( KeyClass key,
	...
	while((tmpEntry!=null) && (LBT.keyCompare(key,tmpEntry.key)>=0)) { 
	  ...
	  if ( leafPage.delEntry(new KeyDataEntry(key, rid)) ) {
		...
		return null;
	      ...
	      // get a sibling
	      LBTIndexPage  parentPage;
	      parentPage =
		new LBTIndexPage(pinPage(parentPageId), 
				headerPage.get_keyType());
	      
	      PageId siblingPageId=new PageId();
	      LBTLeafPage siblingPage;
	      int direction;
		...
		return null;
              ...
              siblingPage=new LBTLeafPage(pinPage(siblingPageId), 
					 headerPage.get_keyType());
		...
		LBTLeafPage leftChild, rightChild;
		if (direction==-1) {
		...
		if ( rightChild.getNextPage().pid != INVALID_PAGE) {
		  LBTLeafPage nextLeafPage=new LBTLeafPage(
	...
	indexPage=new LBTIndexPage(pinPage(currentPageId), headerPage.get_keyType());
	...
	if (indexPage.getCurPage().pid == headerPage.get_rootId().pid) {
	  ...
	  if (indexPage.numberOfRecords() == 0) {
	    LBTSortedPage childPage;
	    childPage=new LBTSortedPage(indexPage.getPrevPage(),
				       headerPage.get_keyType()); 
	...
	if ((4 /*sizeof slot*/ +indexPage.available_space()) <= 
	  ...
	  // get a sibling
	  LBTIndexPage  parentPage;
	  parentPage=new LBTIndexPage(pinPage(parentPageId), 
				     headerPage.get_keyType()); 
	  
	  PageId siblingPageId=new PageId();
	  LBTIndexPage siblingPage;
	  int direction;
	  ...
	  siblingPage=new LBTIndexPage( pinPage(siblingPageId), 
				       headerPage.get_keyType());
	  ...
	  if (siblingPage.redistribute(indexPage,parentPage, 
	    ...
	    // merge the two children
	    LBTIndexPage leftChild, rightChild;
	    if (direction==-1) {
	      leftChild = siblingPage;
   ...
   *@exception UnpinPageException error when unpin a page
   */
  public LBTFileScan new_scan(KeyClass lo_key, KeyClass hi_key)
    throws IOException,  
	   KeyNotMatchException, 
	   IteratorException, 
	   ConstructPageException, 
    ...
    {
      LBTFileScan scan = new LBTFileScan();
      if ( headerPage.get_rootId().pid==INVALID_PAGE) {
	scan.leafPage=null;
	return scan;
      }
  ...
  void trace_children(PageId id)
	...
	KeyClass key;
	KeyDataEntry entry;
	sortedPage=new LBTSortedPage( pinPage( id), headerPage.get_keyType());
	
	
	// Now print all the child nodes of the page.  
	if( sortedPage.getType()==NodeType.INDEX) {
	  LBTIndexPage indexPage=new LBTIndexPage(sortedPage,headerPage.get_keyType()); 
	  ...
	  LBTLeafPage leafPage=new LBTLeafPage(sortedPage,headerPage.get_keyType()); 
