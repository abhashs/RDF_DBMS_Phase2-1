 ...
 *
 */
public class QBT  implements GlobalConst{
  ...
  protected final static byte[] getBytesFromEntry( KeyDataEntry entry ) 
	   ...
	   ConvertException
    {
	  ...
	  n+=4;
        else if (entry.data instanceof QLeafData )      
	  n+=8;
	  ...
	  Convert.setIntValue( ((QLeafData)entry.data).getData().slotNo,
			       m, data);
	  Convert.setIntValue( ((QLeafData)entry.data).getData().pageNo.pid,
			       m+4, data);
  ...
  public static void printPage(PageId pageno, int keyType)
    ...
    {
      QBTSortedPage sortedPage=new QBTSortedPage(pageno, keyType);
      int i;
      i=0;
      if ( sortedPage.getType()==NodeType.INDEX ) {
        QBTIndexPage indexPage=new QBTIndexPage((Page)sortedPage, keyType);
        System.out.println("");
	  ...
	  i++;    
        ...
        QBTLeafPage leafPage=new QBTLeafPage((Page)sortedPage, keyType);
        System.out.println("");
	  ...
	  if( keyType==AttrType.attrInteger) 
	    System.out.println(i+" (key, [pageNo, slotNo]):   ("+ 
			       (IntegerKey)entry.key+ ",  "+(QLeafData)entry.data+ " )");
	  if( keyType==AttrType.attrString) 
	    System.out.println(i+" (key, [pageNo, slotNo]):   ("+ 
			       (StringKey)entry.key + ",  "+(QLeafData)entry.data); 
   ...
   */
  public static void printBTree(QBTreeHeaderPage header) 
    throws IOException, 
	   ConstructPageException, 
	   IteratorException,
	   HashEntryNotFoundException,
  ...
  private static void _printTree(PageId currentPageId, String prefix, int i, 
	   ...
	   ReplacerException 
    {
      ...
      QBTSortedPage sortedPage=new QBTSortedPage(currentPageId, keyType);
      prefix=prefix+"       ";
      i++;
      if( sortedPage.getType()==NodeType.INDEX) {  
	QBTIndexPage indexPage=new QBTIndexPage((Page)sortedPage, keyType);
   ...
   *@exception ReplacerException  error from lower layer
   */
  public static void printAllLeafPages(QBTreeHeaderPage header) 
    throws IOException, 
	   ConstructPageException, 
	   IteratorException,
	   HashEntryNotFoundException,
